...
//    qDebug() << "out 233";
  ...
  connect(timer, &QTimer::timeout, this, [=]() {
    ...
    //    QString time_now = current_date_time.toString("yyyy/MM/dd hh:mm:ss");
    QString time_now = current_date_time.toString("   hh:mm:ss   ");
    show_today->setText(time_now);
...
SystemTool::~SystemTool() { qDebug() << "float close"; }
  ...
  connect(action_animate, &QAction::triggered, this, [=]() {
    if (action_animate->text() == "开始动画") {
      connect(timer2, &QTimer::timeout, robot,
              [=]() {
          robot->move_left_arm(500);

      });
      action_animate->setText("停止动画");
    } else {
//      qDebug() << "stop animate";
      timer->disconnect(robot);
  ...
  connect(action_hide, &QAction::triggered, this,
          [=]() { this->setVisible(false); });
  connect(action_quit, &QAction::triggered, this, [=]() { exit(0); });
